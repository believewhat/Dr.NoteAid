...
def main():
    ...
    data_files["train"] = f"/home/zhichaoyang/pubmedgpt_ct_data/mediqachat_codetosubmit/tmp/preprocessed_data/valid.json"
    data_files["test"] = f"/home/zhichaoyang/pubmedgpt_ct_data/mediqachat_codetosubmit/tmp/preprocessed_data/valid.json"
    datasets_bytask["mediqa_predict"] = load_dataset("json", data_files=data_files, cache_dir="/home/zhichaoyang/pubmedgpt_ct_data/tmp/hfdata", use_auth_token=True if model_args.use_auth_token else None)
    data_files = {}
    data_files["train"] = f"/home/zhichaoyang/pubmedgpt_ct_data/mediqachat_codetosubmit/tmp/preprocessed_data/train.json"
    data_files["test"] = f"/home/zhichaoyang/pubmedgpt_ct_data/mediqachat_codetosubmit/tmp/preprocessed_data/train.json"
    datasets_bytask["mediqa_finetune"] = load_dataset("json", data_files=data_files, cache_dir="/home/zhichaoyang/pubmedgpt_ct_data/tmp/hfdata", use_auth_token=True if model_args.use_auth_token else None)
    ...
    for section_header, indexes in secheader2trainidx.items():
        ...
        if len(indexes) <= num_fewshot_target:
            ...
            subindexes = np.random.choice(indexes, size=num_fewshot_target, replace=False).tolist()
        sectionhead2indexs[section_header] = subindexes
    print(sectionhead2indexs)
    ...
    def preprocess_function_test(examples, index, task_template, datasets_all):
        ...
        input = task_template[0].format(**examples)
        input_with_prompt = " ".join(prompts_fewshot) + input
        src = tokenizer(input_with_prompt, add_special_tokens=True,
